"""
Integration tests for Building Classifier (Phase 2.4).

Tests the integration of BuildingClassifier into the main
...
def test_building_config_initialization():
...
def test_building_config_presets():
...
def test_building_config_to_dict():
...
def test_building_config_from_dict():
...
@pytest.mark.skipif(
    not hasattr(
        __import__(
            ...
            fromlist=["BuildingClassifier"],
        ),
        "BuildingClassifier",
    ),
    reason="BuildingClassifier not available",
)
