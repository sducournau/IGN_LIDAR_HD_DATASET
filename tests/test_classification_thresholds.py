...
class TestThresholdConsistency:
    ...
    def test_road_height_threshold_consistency(self):
        """
        All modules should use the same road height threshold (0.5m).
        ...
        from ign_lidar.core.classification.geometric_rules import (
        ...
        assert gt_config.ROAD_HEIGHT_MAX == 0.5, \
            f"GroundTruthRefinementConfig.ROAD_HEIGHT_MAX should be 0.5, got {gt_config.ROAD_HEIGHT_MAX}"
        assert gt_config.ROAD_HEIGHT_MIN == -0.2, \
        ...
        assert rules_engine.road_vegetation_height_threshold == 0.5, \
        ...
        assert height_thresholds[0] == -0.2, \
            f"Road rule min height should be -0.2, got {height_thresholds[0]}"
        assert height_thresholds[1] == 0.5, \
            f"Road rule max height should be 0.5, got {height_thresholds[1]}"
    ...
    def test_tree_canopy_threshold_consistency(self):
        ...
        gt_config = GroundTruthRefinementConfig()
        assert gt_config.TREE_CANOPY_HEIGHT_MIN == 2.0, \
...
class TestRoadVegetationSeparation:
    ...
    def test_road_surface_classification(self):
        ...
        # Verify statistics
        assert stats['road_validated'] >= 3, "At least 3 road points should be validated"
    ...
    def test_tree_canopy_exclusion(self):
        ...
        if stats.get('road_vegetation_override', 0) > 0:
            ...
            assert refined[4] in [ASPRS_MEDIUM_VEGETATION, ASPRS_HIGH_VEGETATION], \
...
class TestGeometricRulesThresholds:
    ...
    def test_road_vegetation_overlap_fix(self):
        ...
        # Apply rules
        rules_engine = GeometricRulesEngine(
            road_vegetation_height_threshold=0.5
        )
...
class TestIntegration:
    ...
    def test_full_pipeline_consistency(self):
        ...
        assert refiner.config.ROAD_HEIGHT_MAX == 0.5
