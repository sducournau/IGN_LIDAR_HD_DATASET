...
class TestFeatureSignature:
    ...
    def test_vegetation_signature_match(self):
        ...
        signature = FeatureSignature(
            curvature_min=0.15, planarity_max=0.70, ndvi_min=0.20
        )

        # Vegetation-like features
        veg_features = {"curvature": 0.35, "planarity": 0.45, "ndvi": 0.55}
    ...
    def test_vegetation_signature_no_match(self):
        ...
        building_features = {"curvature": 0.05, "planarity": 0.90, "ndvi": 0.10}
    ...
    def test_building_signature_match(self):
        ...
        signature = FeatureSignature(
            curvature_max=0.10, planarity_min=0.70, verticality_min=0.60
        )

        building_features = {"curvature": 0.05, "planarity": 0.85, "verticality": 0.75}
        matches, conf = signature.matches(building_features)
        assert matches is True
        assert conf > 0.9
    ...
    def test_road_signature_match(self):
        ...
        signature = FeatureSignature(
            curvature_max=0.05, planarity_min=0.85, normal_z_min=0.90, height_max=2.0
        )
    ...
    def test_partial_feature_match(self):
        ...
        signature = FeatureSignature(
            curvature_min=0.10, planarity_max=0.60, ndvi_min=0.20
        )

        # Only provide curvature and ndvi (missing planarity)
        partial_features = {"curvature": 0.25, "ndvi": 0.45}
...
class TestFeatureValidator:
    ...
    def test_filter_false_positives(self, validator):
        ...
        ground_truth_types = np.array(
            ["building", "building", "road", "road"], dtype=object
        )
    ...
    def test_custom_config(self):
        ...
        config = {
            ...
            "vegetation_signature": {"curvature_min": 0.20, "ndvi_min": 0.30},
        }
    ...
    def test_mixed_urban_scene(self):
        ...
        features = {
            ...
            "normals": np.vstack(
                [
                    np.column_stack(
                        [np.zeros(100), np.zeros(100), np.random.uniform(0.1, 0.3, 100)]
                    ),
                    np.column_stack(
                        [
                            np.zeros(100),
                            np.zeros(100),
                            np.random.uniform(0.9, 0.99, 100),
                        ]
                    ),
                    np.column_stack(
                        [np.zeros(100), np.zeros(100), np.random.uniform(0.4, 0.7, 100)]
                    ),
                    np.column_stack(
                        [np.zeros(100), np.zeros(100), np.random.uniform(0.3, 0.6, 100)]
                    ),
                    np.column_stack(
                        [np.zeros(100), np.zeros(100), np.random.uniform(0.4, 0.7, 100)]
                    ),
