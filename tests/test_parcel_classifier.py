...
from ign_lidar.core.classification.parcel_classifier import (
...
def test_config_custom():
    ...
    config = ParcelClassificationConfig(
        min_parcel_points=50, parcel_confidence_threshold=0.8, refine_points=False
    )
...
def test_compute_parcel_features_basic(forest_points):
    ...
    stats = classifier.compute_parcel_features(
        parcel_points=points, parcel_features=features, parcel_id="001"
    )
...
def test_compute_parcel_features_missing_features():
    ...
    stats = classifier.compute_parcel_features(
        parcel_points=points, parcel_features=features, parcel_id="test"
    )
...
def test_refine_forest_parcel_points():
    ...
    features = {
        ...
        "height": np.concatenate(
            [np.full(20, 8.0), np.full(30, 1.5), np.full(30, 0.3), np.full(20, 0.0)]
        ),
...
def test_refine_building_parcel_points():
    ...
    features = {
        ...
        "verticality": np.concatenate(
            [np.full(60, 0.8), np.full(40, 0.2)]  # Walls  # Roof
        ),
...
def test_classify_by_parcels_integration(mock_cadastre, forest_points):
    ...
    with patch.object(
        classifier, "_group_by_parcels", return_value={"001": np.arange(100)}
    ):
        labels = classifier.classify_by_parcels(
            points=points, features=features, cadastre=mock_cadastre
        )
