...
from ign_lidar.core.modules.serialization import save_enriched_tile_laz
...
header.offsets = [np.floor(points[:, 0].min()), np.floor(points[:, 1].min()), np.floor(points[:, 2].min())]
...
try:
    ...
    standard = ['X', 'Y', 'Z', 'intensity', 'return_number', 'classification', 'red', 'green', 'blue', 'nir']
