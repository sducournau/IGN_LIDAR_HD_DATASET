...
class TestMultiLevelNDVIRefinement:
    ...
    def test_healthy_trees_classification_with_height(self):
        ...
        labels = np.array([
            engine.ASPRS_ROAD,  # Misclassified
            engine.ASPRS_ROAD,
            engine.ASPRS_ROAD
        ])
...
class TestGeometricRulesIntegration:
    ...
    def test_height_calculation_integration(self):
        ...
        assert updated_labels[2] == engine.ASPRS_HIGH_VEGETATION  # Tall tree (height ≥ 3m)
        assert updated_labels[3] == engine.ASPRS_MEDIUM_VEGETATION  # Tall grass (height ≥ 1m)
...
class TestBackwardCompatibility:
    ...
    def test_legacy_ndvi_vegetation_threshold_parameter(self):
        ...
        engine = GeometricRulesEngine(
            ndvi_vegetation_threshold=0.35  # Legacy value
        )
