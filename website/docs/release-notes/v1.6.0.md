...
**Major Architecture Change**: Data augmentation now happens during the ENRICH phase (before feature computation) instead of the PATCH phase. **Augmentation is now ENABLED BY DEFAULT** - each tile automatically creates 1 original + 3 augmented versions during the enrich process.
