...
VALID_ASPRS_CLASSES.update([40, 41, 42, 43, 44])  # Infra & Agriculture
VALID_ASPRS_CLASSES.update(range(50, 70))  # Buildings (incl. IGN)
...
IGN_CLASS_REMAPPING = {
    64: 6,  # IGN class 64 -> ASPRS Building (rare feature)
    65: 6,  # IGN class 65 -> ASPRS Building (building subtype)
...
def normalize_classification(
    classification: np.ndarray, strict_mode: bool = False, report_unknown: bool = True
    ...
    if report_unknown:
        logger.warning(
            f"⚠️  Found {len(unknown_classes)} non-standard classification codes: {unknown_classes}"
        )
    ...
    for unknown_class in unknown_classes:
        ...
        if unknown_class in IGN_CLASS_REMAPPING:
            ...
            if report_unknown:
                logger.warning(
                    f"   Class {unknown_class}: {count:,} points ({percentage:.2f}%) - NOT REMAPPED (use strict_mode=True)"
                )
            continue
        ...
        if report_unknown:
            logger.info(
                f"   Class {unknown_class} → {target_class}: {count:,} points ({percentage:.2f}%) [{reason}]"
            )
    ...
    if stats["remapped_count"] > 0:
        ...
        logger.info(
            f"✓ Remapped {stats['remapped_count']:,} points ({total_percentage:.2f}%) from {len(stats['remapping_details'])} classes"
        )
...
def validate_classification(
    classification: np.ndarray, allow_extended: bool = True
) -> Dict[str, Any]:
    """
    ...
    if not allow_extended and report["extended_classes"]:
        report["warnings"].append(
            f"Extended classes found but allow_extended=False: {report['extended_classes']}"
        )
        report["is_valid"] = False

    if report["invalid_classes"]:
        report["warnings"].append(
            f"Invalid classification codes found: {report['invalid_classes']}"
        )
...
def get_classification_distribution(
    classification: np.ndarray, class_names: Optional[Dict[int, str]] = None
