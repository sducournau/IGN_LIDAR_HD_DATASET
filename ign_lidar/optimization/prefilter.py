    ...
    def _classify_by_ground_truth_optimized(
        ...
        if height is not None and planarity is not None:
            ...
            if 'roads' in ground_truth_features:
                ...
                logger.info(f"    Road candidates: {len(road_candidates):,} / {len(points):,} "
            ...
            if 'railways' in ground_truth_features:
                ...
                logger.info(f"    Railway candidates: {len(rail_candidates):,} / {len(points):,} "
        ...
        for feature_type, asprs_class in priority_order:
            ...
            if feature_type == 'roads' and road_candidates is not None:
                ...
                logger.info(f"      Using pre-filtered candidates ({len(search_indices):,} points)")
        ...
        if ndvi is not None:
            ...
            if np.any(high_ndvi_buildings):
                ...
                logger.info(f"    Note: {n_veg_on_building} building points with high NDVI")
...
if __name__ == '__main__':
    ...
    print("=" * 80)
    print()
    print("This module provides a quick performance fix for ground truth classification.")
    print()
    print("Usage:")
