...
class OptimizedGroundTruthClassifier:
    ...
    def classify_with_ground_truth(
        ...
        logger.info(f"  Built index with {len(metadata_map)} polygons in {index_time - start_time:.2f}s")
        ...
        labels = self._classify_with_strtree(
            labels, points, tree, metadata_map, candidates_map,
            height, planarity, intensity
        )
    ...
    def _prefilter_candidates(
        ...
        if 'buildings' in ground_truth_features and ground_truth_features['buildings'] is not None:
            ...
            logger.info(f"  Building candidates: {len(candidates_map['buildings']):,} ({reduction:.1f}%)")
    ...
    def _classify_with_strtree(
        ...
        for batch_idx in tqdm(range(n_batches), desc="  Classifying batches", disable=not self.verbose):
    ...
    def _apply_ndvi_refinement(
        ...
        if np.any(high_ndvi_buildings):
            ...
            logger.info(f"  NDVI: {n_veg_on_building:,} building points with high NDVI (vegetation on roofs?)")
