...
Renamed from optimization/gpu.py in v3.3.0 for clarity.
Old import path still works via deprecation stub.
...
Date: November 23, 2025 (Renamed from gpu.py)
Version: 3.3.0
"""
...
# Type checking imports to avoid runtime errors when CuPy not available
if TYPE_CHECKING:
    import cupy as cp
...
class GPUGroundTruthClassifier:
    ...
    def _classify_with_cupy(
        ...
        # OPTIMIZATION: Batch transfer geometric features to GPU
        # Stack features into single array for one transfer instead of 3 separate transfers
        feature_arrays = []
        feature_names = []
        
        if height is not None:
            feature_arrays.append(height)
            feature_names.append('height')
        if planarity is not None:
            feature_arrays.append(planarity)
            feature_names.append('planarity')
        if intensity is not None:
            feature_arrays.append(intensity)
            feature_names.append('intensity')
        
        # Single batched transfer (1 transfer instead of 3)
        if feature_arrays:
            features_stacked = np.stack(feature_arrays, axis=1).astype(np.float32)
            features_gpu = cp.asarray(features_stacked)
            
            # Unpack on GPU (no additional transfer)
            idx = 0
            if height is not None:
                height_gpu = features_gpu[:, idx]
                idx += 1
            if planarity is not None:
                planarity_gpu = features_gpu[:, idx]
                idx += 1
            if intensity is not None:
                intensity_gpu = features_gpu[:, idx]
                idx += 1
