...
class VectorizedGroundTruthClassifier:
    ...
    def classify_with_ground_truth(
        ...
        for chunk_idx in tqdm(
            range(n_chunks), desc="  Chunks", disable=not self.verbose
        ):
            ...
            chunk_planarity = (
                planarity[start_idx:end_idx] if planarity is not None else None
            )
            chunk_intensity = (
                intensity[start_idx:end_idx] if intensity is not None else None
            )
    ...
    def _prefilter_candidates(
        ...
        if (
            ...
            logger.info(
                f"    Road candidates: {len(candidates_map['roads']):,} ({len(candidates_map['roads'])/len(points)*100:.1f}%)"
            )
        ...
        if (
            ...
            logger.info(
                f"    Railway candidates: {len(candidates_map['railways']):,} ({len(candidates_map['railways'])/len(points)*100:.1f}%)"
            )
        ...
        if (
            ...
            logger.info(
                f"    Building candidates: {len(candidates_map['buildings']):,} ({len(candidates_map['buildings'])/len(points)*100:.1f}%)"
            )
    ...
    def _classify_chunk(
        ...
        for feature_type, asprs_class in priority_order:
            ...
            if feature_type in candidates_map:
                ...
                chunk_candidates_mask = (global_candidates >= chunk_start) & (
                    global_candidates < chunk_end
                )
                chunk_candidate_indices = (
                    global_candidates[chunk_candidates_mask] - chunk_start
                )
            ...
            if feature_type == "roads" and self.road_buffer_tolerance > 0:
                feature_gdf["geometry"] = feature_gdf["geometry"].buffer(
            ...
            try:
                joined = gpd.sjoin(
                    filtered_points_gdf, feature_gdf, how="inner", predicate="within"
                )
