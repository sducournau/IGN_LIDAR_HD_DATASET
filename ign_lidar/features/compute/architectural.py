...
def compute_architectural_features(
    ...
    if points.shape[0] != normals.shape[0] or points.shape[0] != eigenvalues.shape[0]:
        raise ValueError(
            "points, normals, and eigenvalues must have same number of rows"
        )
...
def compute_planarity(eigenvalues: np.ndarray, epsilon: float = 1e-6) -> np.ndarray:
    ...
    planarity : np.ndarray
        Planarity values of shape (N,), range [0, 1]
    """
    ...
    # Compute and clamp to valid range
...
def compute_building_regularity(
    eigenvalues: np.ndarray, epsilon: float = 1e-10
) -> np.ndarray:
    """
