...
class BaseFeatureStrategy(ABC):
    ...
    def __init__(
        self, k_neighbors: int = 20, radius: float = 1.0, verbose: bool = False
    ):
        """
    ...
    def compute_geometric_features(
        self, points: np.ndarray, intensities: Optional[np.ndarray] = None
    ) -> Dict[str, np.ndarray]:
        """
    ...
    def compute_rgb_features(self, rgb: np.ndarray) -> Dict[str, np.ndarray]:
        """
    ...
    @staticmethod
    def auto_select(
        n_points: int, mode: str = "auto", has_gpu: Optional[bool] = None, **kwargs
    ) -> "BaseFeatureStrategy":
        """
        ...
        if mode == "cpu":
            ...
            if not has_gpu:
                logger.warning(
                    "GPU chunked requested but GPU not available, falling back to CPU"
                )
                return CPUStrategy(**kwargs)
            try:
                ...
                logger.info(
                    f"Using GPU chunked strategy (forced) for {n_points:,} points"
                )
            ...
            if n_points < 1_000_000:
                logger.info(
                    f"Auto-selected CPU strategy for {n_points:,} points (< 1M)"
                )
                return CPUStrategy(**kwargs)
            ...
            if not has_gpu:
                logger.info(
                    f"Auto-selected CPU strategy for {n_points:,} points (no GPU)"
                )
                return CPUStrategy(**kwargs)
            ...
            if n_points < 10_000_000:
                try:
                    ...
                    logger.info(
                        f"Auto-selected GPU strategy for {n_points:,} points (1M-10M)"
                    )
                    return GPUStrategy(**kwargs)
                except ImportError:
                    logger.warning("GPU strategy not available, falling back to CPU")
            ...
            # Large datasets with GPU: chunked GPU
                ...
                try:
                    ...
                    logger.info(
                        f"Auto-selected GPU chunked strategy for {n_points:,} points (> 10M)"
                    )
                    return GPUChunkedStrategy(**kwargs)
                except ImportError:
                    logger.warning(
            ...
            raise ValueError(
                f"Unknown compute mode: {mode}. Use 'cpu', 'gpu', 'gpu_chunked', or 'auto'"
            )
