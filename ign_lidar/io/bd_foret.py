...
class BDForetFetcher:
    ...
    def __init__(
        self, cache_dir: Optional[Path] = None, config: Optional[BDForetConfig] = None
    ):
        """
    ...
    def _fetch_wfs_layer(
        ...
        if self.cache_dir:
            ...
            if cache_file.exists():
                try:
                    ...
                    logger.debug(
                        f"Loaded {len(gdf)} forest formations from cache (skipped WFS fetch)"
                    )
                    return gdf
                except Exception as e:
                    logger.warning(
        ...
        try:
            response = requests.get(self.config.WFS_URL, params=params, timeout=60)
            ...
            gdf = gpd.GeoDataFrame.from_features(
                geojson_data["features"], crs=self.config.CRS
            )
    ...
    def _process_forest_attributes(self, gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        """
    ...
    def label_points_with_forest_type(
        ...
        try:
            ...
            if valid_forests:
                ...
                for i, point_geom in enumerate(point_geoms):
                    ...
                    candidate_forests = forest_tree.query(
                        point_geom, predicate="contains"
                    )
            ...
            logger.warning(
                f"  STRtree optimization failed ({e}), falling back to nested loop"
            )
        ...
        logger.info(
            f"  Labeled {n_labeled:,} points ({100*n_labeled/n_points:.1f}%) with forest data"
        )
...
def fetch_and_label_forest(
    ...
    labels, forest_attrs = fetcher.label_points_with_forest_type(
        points=points, forest_gdf=forest_gdf, existing_labels=existing_labels
    )
