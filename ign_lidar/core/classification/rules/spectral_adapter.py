...
class SpectralRulesAdapter(LegacyEngineAdapter):
    ...
    def __init__(
        ...
        logger.info(f"Created SpectralRulesAdapter for target class {config.target_class}")
    ...
    def _compute_spectral_confidence(
        ...
        if self.target_class in [3, 4, 5]:  # Vegetation
            ...
            if ndvi is not None:
                ...
                conf_values = np.clip(
                    nir_matched * 0.5 + ndvi_matched * 0.5,
                    0.5, 0.95
                )
            else:
            ...
            conf_values = np.clip(
                nir_score * 0.4 + brightness_score * 0.6,
                0.5, 0.95
            )
            ...
            brightness = np.mean(rgb_matched, axis=1)
            conf_values = np.clip(
                (1.0 - nir_matched) * 0.5 + (1.0 - brightness) * 0.5,
                0.5, 0.95
            )
...
def create_spectral_vegetation_rule(
    ...
    return SpectralRulesAdapter(
        config=config,
        nir_vegetation_threshold=nir_threshold
    )
