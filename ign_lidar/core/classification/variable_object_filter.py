...
class VariableObjectFilter:
    ...
    def __init__(
        ...
        if self.filter_vehicles:
            logger.info(
                f"  🚗 Vehicle filtering: {self.vehicle_height_min}-{self.vehicle_height_max}m"
            )
        if self.filter_urban_furniture:
            logger.info(
                f"  🪑 Furniture filtering: {self.furniture_height_min}-{self.furniture_height_max}m, max {self.furniture_max_cluster} pts"
            )
        if self.filter_walls:
            logger.info(
                f"  🧱 Wall filtering: {self.wall_height_min}-{self.wall_height_max}m, verticality>{self.wall_min_verticality}"
            )
    ...
    def filter_variable_objects(
        ...
        # 1. Filter vehicles
        if self.filter_vehicles:
            # Extract NDVI and is_ground if available
            ndvi = features.get("ndvi") if features else None
            is_ground = features.get("is_ground") if features else None
        ...
        if self.filter_urban_furniture:
            n_furn = self._filter_urban_furniture(
                points, classification_filtered, height_above_ground
            )
        ...
        if self.filter_walls and features is not None:
            n_walls = self._filter_walls(
                classification_filtered, height_above_ground, features
            )
        ...
        if stats["total_filtered"] > 0:
            ...
            logger.info(
                f"  ✅ Total variable objects filtered: {stats['total_filtered']:,} points ({pct:.2f}%)"
            )
        else:
    ...
    def _filter_vehicles(
        ...
        Args:
            ...
            ndvi: Optional NDVI values for vegetation exclusion
        ...
        if is_ground is not None:
            ...
            if n_ground_rejected > 0 and self.verbose:
                logger.debug(
                    f"    Rejected {n_ground_rejected:,} ground points "
                    f"from vehicle candidates"
                )
        ...
        if ndvi is not None:
            ...
            n_vegetation_rejected = np.sum(vehicle_mask & vegetation_mask)
            if n_vegetation_rejected > 0 and self.verbose:
                logger.debug(
                    f"    Rejected {n_vegetation_rejected:,} vegetation points "
                    f"(NDVI > {vegetation_threshold}) from vehicle candidates"
                )
            vehicle_mask &= ~vegetation_mask  # Vehicles are not vegetation
        ...
        if n_vehicles > 0:
            ...
            if self.create_vehicle_class:
                ...
                if self.verbose:
                    logger.debug(
                        f"    Created vehicle class {self.vehicle_class_code}: "
                        f"{n_vehicles:,} points"
                    )
            else:
                ...
                if self.verbose:
                    logger.debug(
                        f"    Reclassified to {self.reclassify_to}: "
                        f"{n_vehicles:,} points"
                    )
    ...
    def _filter_urban_furniture(
        self, points: np.ndarray, classification: np.ndarray, height: np.ndarray
    ) -> int:
        """
        ...
        furniture_height_mask = (height >= self.furniture_height_min) & (
        ...
        try:
            ...
            if len(furniture_indices) > 0:
                ...
                if self.verbose:
                    logger.debug(
                        f"    Small clusters (<{self.furniture_max_cluster} pts): {len(furniture_indices):,} points"
                    )
...
def apply_variable_object_filtering(
    ...
    if height_above_ground is None:
        logger.warning(
            "  ⚠️  height_above_ground not available - skipping variable object filtering"
        )
        logger.warning(
            "      Enable DTM computation: features.compute_height_above_ground = true"
        )
        return classification, {"total_filtered": 0}
    ...
    filter = VariableObjectFilter(
        ...
        vehicle_height_range=tuple(
            filter_config.get("vehicle_height_range", [0.8, 4.0])
        ),
        furniture_height_range=tuple(
            filter_config.get("furniture_height_range", [0.5, 4.0])
        ),
...
__all__ = ["VariableObjectFilter", "apply_variable_object_filtering"]
