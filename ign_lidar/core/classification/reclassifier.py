...
class OptimizedReclassifier:
    ...
    def __init__(
        ...
        if acceleration_mode == "auto":
            ...
            if acceleration_mode in ["gpu", "gpu+cuml"] and not HAS_GPU:
                logger.warning(f"GPU mode requested but RAPIDS not available, falling back to CPU")
                self.acceleration_mode = "cpu"
    ...
    def reclassify(
        ...
        logger.info(f"  Total points changed: {n_changed:,} ({100*n_changed/n_points:.2f}%)")
    ...
    def _classify_feature_gpu(
        ...
        try:
            ...
            points_gpu = cudf.DataFrame({
                'x': cp.asarray(points[:, 0]),
                'y': cp.asarray(points[:, 1])
            })
            ...
            for i in range(n_chunks):
                ...
                for poly_coords in polygon_list:
                    ...
                    try:
                        ...
                        logger.debug(f"GPU polygon test failed, using CPU fallback: {e}")
                        continue
            ...
            logger.warning(f"GPU classification failed for {feature_name}, falling back to CPU: {e}")
            # Fallback to CPU implementation
            return self._classify_feature_cpu(points, labels, geometries, asprs_code, feature_name)
    ...
    def reclassify_file(
        ...
        try:
            if hasattr(las, 'ndvi'):
                ...
                logger.info(f"  Found NDVI data (range: {ndvi.min():.3f} to {ndvi.max():.3f})")
