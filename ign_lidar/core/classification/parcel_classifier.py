...
class ParcelClassifier:
    ...
    def classify_by_parcels(
        ...
        if not parcel_groups:
            logger.warning("   No points assigned to parcels, falling back to unclassified")
            return labels
        ...
        for parcel_id, point_indices in parcel_groups.items():
            ...
            if rpg is not None:
                parcel_stats.rpg_match = self._match_rpg(
                    parcel_id,
                    parcel_points,
                    rpg
                )
    ...
    def compute_parcel_features(
        ...
        stats = ParcelStatistics(
            ...
            dominant_normal_z=float(np.median(normals[:, 2])) if len(normals) > 0 else 0.0
        )
    ...
    def classify_parcel_type(
        ...
        if self.config.agriculture_ndvi_min <= stats.mean_ndvi <= self.config.agriculture_ndvi_max:
    ...
    def _match_bd_foret(
        ...
        centroid = Point(
            np.mean(parcel_points[:, 0]),
            np.mean(parcel_points[:, 1])
        )
        ...
        return [
            {
                ...
                'confidence': max(stats.confidence_scores.values()) if stats.confidence_scores else 0.0
            }
