...
class SpectralRulesEngine:
    ...
    def __init__(
        ...
        logger.info(f"   Brightness concrete range: [{brightness_concrete_min}, {brightness_concrete_max}]")
        logger.info(f"   NIR/Red ratio vegetation threshold: {nir_red_ratio_veg_threshold}")
    ...
    def classify_by_spectral_signature(
        ...
        stats['building_concrete_spectral'] = np.sum(concrete_mask)
        ...
        labels[metal_mask] = self.ASPRS_BUILDING
        stats['building_metal_spectral'] = np.sum(metal_mask)
        ...
        stats['road_asphalt_spectral'] = np.sum(asphalt_mask)
        ...
        if stats['total_reclassified'] > 0:
            logger.info(f"  🌈 Spectral rules classified {stats['total_reclassified']:,} points:")
            if stats['vegetation_spectral'] > 0:
                logger.info(f"     Vegetation (spectral): {stats['vegetation_spectral']:,}")
            if stats['water_spectral'] > 0:
                logger.info(f"     Water (spectral): {stats['water_spectral']:,}")
            if stats['building_concrete_spectral'] > 0:
                logger.info(f"     Building/Concrete (spectral): {stats['building_concrete_spectral']:,}")
    ...
    def classify_with_confidence(
        ...
        veg_confidence = np.clip(
            (ndvi - 0.3) / 0.3 + (features['nir'] - self.nir_vegetation_threshold) / 0.3,
            0, 1
        )
