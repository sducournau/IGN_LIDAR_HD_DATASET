...
v3.2+ Changes:
    ...
    - classify_from_asprs() maintains original functionality
...
class ClassificationResult:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "class_percentages": {
                int(c): float(cnt) / total * 100 for c, cnt in zip(unique, counts)
            },
...
class HierarchicalClassifier(BaseClassifier):
    ...
    - Now inherits from BaseClassifier for API consistency
    - Added classify() method following BaseClassifier interface
    - classify_from_asprs() wraps original functionality
    ...
    Example (legacy interface - still works):
        >>> classifier = HierarchicalClassifier(target_level=ClassificationLevel.LOD2)
        >>> result = classifier.classify_from_asprs(asprs_labels, features)
    """
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized HierarchicalClassifier (target level: {target_level.name})"
        )
        ...
        **kwargs,
        ...
        Args:
            ...
            features: Dictionary mapping feature name → array [N]
                REQUIRED: 'asprs_labels' - initial ASPRS classification
                Optional: 'height', 'ndvi', 'normals', 'planarity', etc.
            ground_truth: Optional ground truth data for refinement
            **kwargs: Additional parameters:
                - track_hierarchy: bool (track classification path)

        Returns:
            BaseClassificationResult with labels, confidence, and metadata

        Raises:
            ValueError: If 'asprs_labels' not in features
        ...
        # Check for required asprs_labels
        if "asprs_labels" not in features:
            raise ValueError(
        ...
        # Extract other features (excluding asprs_labels)
        other_features = {k: v for k, v in features.items() if k != "asprs_labels"}

        # Call original classify_from_asprs method
        track_hierarchy = kwargs.get("track_hierarchy", False)
        result = self.classify_from_asprs(
            asprs_labels=asprs_labels,
            features=other_features if other_features else None,
            ground_truth=ground_truth,
            track_hierarchy=track_hierarchy,
        )

        # Convert to BaseClassificationResult
    ...
    def classify_from_asprs(
        ...
        if features is not None:
            final_labels, refined_count, feature_importance = (
        ...
        logger.info(
            f"Classification complete: {stats['total_points']:,} points, "
    ...
    def _initialize_mappings(self):
        ...
        self.asprs_to_lod3 = (
            ASPRS_TO_LOD3 if hasattr("ASPRS_TO_LOD3", "__module__") else {}
        )
    ...
    def _compute_mapping_confidence(
        self, source_labels: np.ndarray, target_labels: np.ndarray, level: str
    ) -> np.ndarray:
        """
    ...
    def _refine_classification(
        ...
        if "ndvi" in features:
            ndvi = features["ndvi"]
            refined, count = self._refine_by_ndvi(refined, ndvi, features.get("height"))
        ...
        logger.debug(
            f"Refinement summary: height={refined_by_height}, "
    ...
    def _refine_by_height(
        self, labels: np.ndarray, height: np.ndarray
    ...
    def _refine_by_ndvi(
        self, labels: np.ndarray, ndvi: np.ndarray, height: Optional[np.ndarray]
    ) -> Tuple[np.ndarray, int]:
    ...
    def _refine_by_ground_truth(
        ...
        logger.debug(
            "Ground truth refinement not yet implemented in hierarchical classifier"
        )
...
def classify_hierarchical(
    ...
    classifier = HierarchicalClassifier(
        target_level=level, use_confidence_scores=use_confidence
    )
