...
class GroundTruthRefinementConfig:
    ...
    # Road refinement
    ROAD_HEIGHT_MAX = 2.0            # Maximum height for roads (meters)
    ROAD_HEIGHT_MIN = -0.5           # Minimum height (handle slight variations)
...
class GroundTruthRefiner:
    ...
    def refine_water_classification(
        ...
        if planarity is not None:
            planarity_valid = planarity[water_candidates] >= self.config.WATER_PLANARITY_MIN
        ...
        if curvature is not None:
            curvature_valid = curvature[water_candidates] <= self.config.WATER_CURVATURE_MAX
    ...
    def refine_road_classification(
        ...
        if height is not None:
            height_valid = (
                (height[road_candidates] >= self.config.ROAD_HEIGHT_MIN) & 
                (height[road_candidates] <= self.config.ROAD_HEIGHT_MAX)
            )
            
            # Special case: Points above road are likely tree canopy over road
            above_road = height[road_candidates] > self.config.ROAD_HEIGHT_MAX
            
            valid_road &= height_valid
            n_height_invalid = np.sum(~height_valid)
            if n_height_invalid > 0:
                validation_reasons.append(f"height: {n_height_invalid} rejected (likely tree canopy)")
        
        # Criterion 2: Planarity (roads should be very flat)
        if planarity is not None:
            planarity_valid = planarity[road_candidates] >= self.config.ROAD_PLANARITY_MIN
        ...
        if curvature is not None:
            curvature_valid = curvature[road_candidates] <= self.config.ROAD_CURVATURE_MAX
        ...
        if ndvi is not None:
            ...
            if height is not None:
                # High NDVI + elevated = tree canopy
                tree_canopy = high_ndvi & (height[road_candidates] > 2.0)
        ...
        if stats['road_vegetation_override'] > 0:
            logger.info(f"    🌳 Tree canopy over road: {stats['road_vegetation_override']:,} points → vegetation")
    ...
    def refine_vegetation_with_features(
        ...
        logger.info("  Refining vegetation classification with NDVI + curvature + sphericity...")
        ...
        if planarity is not None:
            ...
            plan_normalized = 1.0 - np.clip(planarity / self.config.VEG_PLANARITY_MAX, 0, 1)
        ...
        if height is not None:
            ...
            med_veg_mask = is_vegetation & (height > self.config.VEG_LOW_HEIGHT_MAX) & (height <= self.config.VEG_MEDIUM_HEIGHT_MAX)
    ...
    def refine_building_with_expanded_polygons(
        ...
        logger.info(f"    Found {len(building_candidates):,} candidates in expanded polygons")
        ...
        if height is not None:
            height_valid = height[building_candidates] >= self.config.BUILDING_HEIGHT_MIN
        ...
        if planarity is not None and verticality is not None:
            ...
            geometry_valid = (
                ...
                (verticality[building_candidates] >= self.config.BUILDING_VERTICAL_THRESHOLD)
            )
            valid_building &= geometry_valid
            n_geometry_invalid = np.sum(~geometry_valid)
            if n_geometry_invalid > 0:
                logger.info(f"      - Geometry: {n_geometry_invalid} rejected (neither flat nor vertical)")
        ...
        if ndvi is not None:
            ...
            if n_ndvi_invalid > 0:
                logger.info(f"      - NDVI: {n_ndvi_invalid} rejected (likely vegetation)")
        ...
        stats['building_validated'] = np.sum(labels[validated_indices] == self.ASPRS_BUILDING)
        stats['building_expanded'] = np.sum(labels[validated_indices] != self.ASPRS_BUILDING)
        stats['building_rejected'] = len(rejected_indices)
        
        # Log results
        logger.info(f"    ✓ Expanded buildings: {stats['building_expanded']:,} new building points")
        logger.info(f"    ✓ Total validated: {stats['building_validated'] + stats['building_expanded']:,} building points")
    ...
    def refine_all(
        ...
        if 'roads' in ground_truth_features:
            ...
            if roads_gdf is not None and len(roads_gdf) > 0:
                ...
                refined, road_stats = self.refine_road_classification(
                    refined, points, road_mask, height, planarity, curvature, normals, ndvi
                )
        ...
        logger.info("  Using feature-based vegetation classification (NDVI + curvature + sphericity)")
