...
class AdaptiveBuildingClassifier:
    ...
    def __init__(
        ...
        """
        self.signature = signature if signature is not None else BuildingFeatureSignature()
        ...
        logger.info(f"  Fuzzy boundaries: inner={fuzzy_boundary_inner}m, outer={fuzzy_boundary_outer}m")
        logger.info(f"  Adaptive expansion: {enable_adaptive_expansion} (max {max_expansion_distance}m)")
        logger.info(f"  Intelligent rejection: {enable_intelligent_rejection}")
        logger.info(f"  Spatial clustering: {enable_spatial_clustering} (radius {spatial_radius}m)")
    ...
    def classify_buildings_adaptive(
        ...
        """
        if not HAS_SPATIAL:
            raise ImportError("Shapely and GeoPandas required for adaptive classification")
        
        logger.info("=== Adaptive Building Classification ===")
        logger.info(f"Processing {len(points):,} points with {len(building_polygons)} building polygons")
        ...
        logger.info("  Step 1/5: Computing height scores...")
        height_scores = self._compute_height_scores(height)
        
        # Step 2: Compute geometry scores (walls + roofs)
        logger.info("  Step 2/5: Computing geometry scores...")
        geometry_scores, is_wall, is_roof = self._compute_geometry_scores(
            planarity, verticality, curvature, normals
        )
        
        # Step 3: Compute spectral scores (NDVI)
        logger.info("  Step 3/5: Computing spectral scores...")
        spectral_scores = self._compute_spectral_scores(ndvi, intensity)
        
        # Step 4: Compute ground truth proximity scores (fuzzy boundaries)
        logger.info("  Step 4/5: Computing ground truth proximity scores...")
        ...
        logger.info("  Step 5/5: Computing spatial coherence scores...")
        ...
        logger.info("  Computing overall confidence scores...")
        ...
        for i in range(n_points):
            ...
            if inside_polygon:
                ...
                elif self.enable_intelligent_rejection and confidences[i] < self.rejection_confidence_threshold:
                ...
                if self.enable_adaptive_expansion:
                    ...
                    if (distance <= self.max_expansion_distance and
                        confidences[i] >= self.expansion_confidence_threshold):
        ...
        logger.info(f"Total classified as building: {total_classified:,} ({stats['classification_rate']:.1f}%)")
        ...
        if self.enable_intelligent_rejection:
            logger.info(f"  Rejected (inside but low confidence): {stats['rejected']:,} points")
    ...
    def _compute_height_scores(
        ...
        # Perfect score for typical building heights
        typical_mask = (
        ...
        if np.any(elevated_mask):
            ...
            above_mask = elevated_mask & (height > self.signature.typical_height_range[1])
            ...
            between_mask = elevated_mask & (height <= self.signature.typical_height_range[1])
        ...
        if np.any(below_mask):
            scores[below_mask] = np.clip(height[below_mask] / self.signature.min_height, 0.0, 0.5)
    ...
    def _compute_geometry_scores(
        ...
        if planarity is not None:
            ...
            if curvature is not None:
                curv_bonus = np.clip(
                    1.0 - curvature / self.signature.roof_curvature_max,
                    0.0, 1.0
                )
            ...
            if normals is not None:
                ...
                normal_valid = (
                    (normal_z >= self.signature.roof_normal_z_range[0]) &
    ...
    def _compute_spectral_scores(
        ...
        if ndvi is not None:
            ndvi_score = np.clip(
                1.0 - ndvi / self.signature.ndvi_max,
                0.0, 1.0
            )
            scores = ndvi_score
        else:
    ...
    def _compute_spatial_coherence_scores(
        ...
        try:
            ...
            for i in range(n):
                ...
                scores[i] = min(1.0, neighbor_ratio / self.min_neighbor_ratio) * avg_neighbor_quality
