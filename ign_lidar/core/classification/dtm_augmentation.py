...
class DTMAugmentationConfig:
    ...
    max_height_difference: float = 5.0   # Max difference from nearby real ground (meters)
    ...
    def log_summary(self, logger):
        ...
        if self.total_validated > 0:
            ...
            if self.vegetation_points > 0:
                ...
                logger.info(f"    - Vegetation: {self.vegetation_points:,} ({pct:.1f}%)")
        ...
        if self.total_rejected > 0:
            ...
            if self.rejected_height_diff > 0:
                ...
                logger.info(f"    - Height difference: {self.rejected_height_diff:,} ({pct:.1f}%)")
            if self.rejected_spacing > 0:
                pct = 100 * self.rejected_spacing / self.total_rejected
                logger.info(f"    - Too close to existing: {self.rejected_spacing:,} ({pct:.1f}%)")
            if self.rejected_no_neighbors > 0:
                pct = 100 * self.rejected_no_neighbors / self.total_rejected
                logger.info(f"    - No neighbors for validation: {self.rejected_no_neighbors:,} ({pct:.1f}%)")
...
class DTMAugmenter:
    ...
    def augment_point_cloud(
        ...
        synthetic_points = self._generate_synthetic_points(
            dtm_fetcher, bbox, crs
        )
        ...
        self.stats.total_rejected = self.stats.total_generated - self.stats.total_validated
    ...
    def _filter_under_buildings(
        ...
        """
        if not HAS_GEOPANDAS or building_polygons is None or len(building_polygons) == 0:
            return np.array([]).reshape(0, 3), 0
