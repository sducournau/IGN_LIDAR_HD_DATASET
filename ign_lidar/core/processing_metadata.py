    ...
    def compute_config_hash(self, config: DictConfig) -> str:
        ...
        relevant_config = {
            'processor': {
                ...
                'building_detection_mode': config.processor.get('building_detection_mode', 'asprs'),
                'transport_detection_mode': config.processor.get('transport_detection_mode', 'asprs_standard'),
            },
            'features': {
            ...
            'data_sources': {
                'bd_topo': {
                    ...
                    'features': OmegaConf.to_container(config.data_sources.bd_topo.features),
                    'parameters': OmegaConf.to_container(config.data_sources.bd_topo.get('parameters', {})),
                },
                'bd_foret': {
            ...
            'ground_truth': {
                ...
                'post_processing': OmegaConf.to_container(config.ground_truth.get('post_processing', {})),
            },
        }
    ...
    def get_processing_stats(self) -> Dict[str, Any]:
        ...
        for metadata_file in metadata_files:
            try:
                ...
                stats['total_processing_time'] += metadata.get('processing_time_seconds', 0)
                stats['total_points'] += metadata.get('num_points', 0)
                stats['config_hashes'].add(metadata.get('config_hash', ''))
                
                timestamp = metadata.get('timestamp')
                if timestamp:
                    if stats['oldest_processing'] is None or timestamp < stats['oldest_processing']:
                        stats['oldest_processing'] = timestamp
                    if stats['newest_processing'] is None or timestamp > stats['newest_processing']:
        ...
        for metadata_file in self.metadata_dir.glob("*.json"):
            ...
            if not has_output:
                try:
                    ...
                    logger.warning(f"Failed to remove orphaned metadata {metadata_file}: {e}")
