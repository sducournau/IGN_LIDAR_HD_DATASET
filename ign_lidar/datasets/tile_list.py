...
def get_url_mapping() -> Dict[str, str]:
    ...
    return {tile.filename: tile.download_url for tile in WORKING_TILES if tile.download_url}


# Statistics
print(f"Loaded {len(WORKING_TILES)} tiles from real WFS data")
print(f"Environments: {len(set(tile.environment for tile in WORKING_TILES))}")
print(f"Regions: {len(set(tile.location.split(' - ')[0] for tile in WORKING_TILES))}")
